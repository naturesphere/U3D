...
public class CreateScoreGoal : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider collider)
    {
        if (Staticer.STATUS != Staticer.NORMAL)
            return;
        Vector2 ballPosition = new Vector2(GameObject.FindWithTag("ball").transform.position.x,
            GameObject.FindWithTag("ball").transform.position.y);
        if(collider.gameObject.tag=="ball")
        {
            float lengthToCenterPow = Mathf.Pow(ballPosition.x - Staticer.goalX, 2) + Mathf.Pow(ballPosition.y - Staticer.goalY, 2);
            if (lengthToCenterPow <= 0.058)
